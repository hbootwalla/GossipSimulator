...
defmodule FullGossip do
    ...
    def spawnFullActors(numOfNodes, nodeCount, totRepeat, list)  do
      if nodeCount != numOfNodes do
        pid = spawn(__MODULE__, :startFullGossip, [1, totRepeat, nil]);
        list = list ++ [pid];
        spawnFullActors(numOfNodes, nodeCount + 1,totRepeat, list)
      else
        list
      end    
    end
    ...
    def startFullGossip(count, totRepeat, agent_link) when count <= totRepeat do
      receive do
        {:add_neighbors, n_pid} ->  {:ok, agent_link} = FullAgent.start_link(n_pid);
                                    startFullGossip(count, totRepeat, agent_link);
        ...
        {:gossip, ppid, message} -> IO.puts(List.to_string(:erlang.pid_to_list(ppid)) <> " --> " <> List.to_string(:erlang.pid_to_list(self())) <> " Count: #{count}");
                                    ...
                                    case neighborPid do
                                        ...
                                        _ -> send(neighborPid, {:gossip, self(), message});
                                              ...
                                              startFullGossip(count+1,totRepeat,agent_link);
                                    end 
      end
    end
